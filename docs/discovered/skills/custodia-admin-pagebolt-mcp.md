---
title: "Custodia-Admin/pagebolt-mcp — Claude Code skill discovered by cue"
description: "An MCP server to allow AI agents to interact with PageBolt to take screenshots, grab PDFs, and more."
layout: page
updated: 2026-05-31
tags: [claude-code, skill, backend]
---

# ✨ [Custodia-Admin/pagebolt-mcp](https://github.com/Custodia-Admin/pagebolt-mcp)

**★ 1** · strong (score 7.8) · JavaScript · ai-agents, browser-automation, mcp, mcp-server, mcp-servers

> An MCP server to allow AI agents to interact with PageBolt to take screenshots, grab PDFs, and more.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 7.8 based on:
- ⭐ 1 stars
- 🗓️ Last pushed: 2026-05-31
- 🏷️ Tags: ai-agents, browser-automation, mcp, mcp-server, mcp-servers, pdf-generation, puppeteer, screenshot-api

## Best fit cue profiles

[backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add Custodia-Admin/pagebolt-mcp --profile backend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/claude-code-skills) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/Custodia-Admin/pagebolt-mcp)
