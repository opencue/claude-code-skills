---
title: "Abhigyan-Shekhar/Waggle-mcp — Claude Code skill discovered by cue"
description: "MCP server for external memory layer for AI agents + more . Download from pypi , and get started "
layout: page
updated: 2026-05-31
tags: [claude-code, skill, core]
---

# 💎 [Abhigyan-Shekhar/Waggle-mcp](https://github.com/Abhigyan-Shekhar/Waggle-mcp)

**★ 8** · exceptional (score 9.1) · Python · agent-memory, ai-agents, graph-memory, knowledge-graph, llm-memory

> MCP server for external memory layer for AI agents + more . Download from pypi , and get started 

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 9.1 based on:
- ⭐ 8 stars
- 🗓️ Last pushed: 2026-05-31
- 🏷️ Tags: agent-memory, ai-agents, graph-memory, knowledge-graph, llm-memory, mcp, mcp-server, python

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add Abhigyan-Shekhar/Waggle-mcp --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/claude-code-skills) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/Abhigyan-Shekhar/Waggle-mcp)
